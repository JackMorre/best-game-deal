...
export const SelectedGame = () => {
  ...
  const getOptions = async () => {
    try {
      const res = await axios.get(
        `https://www.cheapshark.com/api/1.0/deals?id=0f%2B4gT2VVUn4UcmFzPxXnuqoXKAOYoJ5mpFZRWNyohc%3D`
      );
      const resData = res.data;
      setOptions(resData.cheaperStores);
      setInfo(resData.gameInfo);
  ...
  return (
    <Stack className="body-page">
      ...
      <SelectedGameThumbnail header={info?.name} thumbnail={info?.thumb} />
      <div className="row">
        <div className="col">
          {optionLoading &&
            options.map((option) => {
